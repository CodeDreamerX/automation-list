...
/**
 * Get the icon URL for a given icon name
...
// Map old technology icon names (with icon- prefix or different names) to actual file names
const technologyIconNameMap: Record<string, string> = {
  // Old names with icon- prefix
  'icon-robot': 'robot',
  'icon-scada': 'scada-hmi',
  'icon-oem': 'oem',
...
export function getTechnologyIconUrl(iconName: string | null | undefined): string {
  ...
  // Check if we have a mapping for this icon name
  const mappedName = technologyIconNameMap[trimmed.toLowerCase()];
  if (mappedName) {
    return `/icons/categories/${mappedName}.svg`;
  }
